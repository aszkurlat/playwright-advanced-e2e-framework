...
import HomePage from "../page-objects/nopcommerce-login.page.js";
import CustList from "../page-objects/nopcommerce-customer-list.page.js";
...
test.describe("E2E Customer Search", () => {
    test("E2E_TC001: Search the external customers in customer portal", async ({ page, request }, testInfo) => {
        ...
        /**1. Get list of users */
        // Make a GET call
        await logWithStep("info", `Making a GET call using ${envConfig.apiURL}`);
        ...
        for (const user of USER_DATA) {
            ...
            if (customerNotFound) {
                await logWithStep("warn", `The giver user: ${user.first_name} ${user.last_name} could not found in the portal`);
            } else {
                await logWithStep("info", `The giver user: ${user.first_name} ${user.first_name} found in the portal`);
            }
        }
    });
});